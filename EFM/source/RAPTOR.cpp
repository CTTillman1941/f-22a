...
void ed_fm_simulate(double dt) {
    ...
    if (RAPTOR::fm_clock < 0.1) {
        ...
        constexpr double takeoff_trim_value = 0.25;
        ...
        double yaw_cmd;
        constexpr double beta_integral_decay = 0.07;
        ...
        if (fabs(yaw_cmd_source) < 0.01) {
            ...
            if (fabs(RAPTOR::beta) < 3.0 && !RAPTOR::on_ground && fabs(RAPTOR::rudder_command) < 0.9) {
	            constexpr double beta_integral_gain = 0.006;
	            constexpr double beta_integral_limit = 0.01;
...
void ed_fm_set_draw_args_v2(float* data, const size_t size) {
    ...
    if (RAPTOR::landing_brake_assist >= 1.0f && RAPTOR::on_ground) {
        ...
        const float target_left_aileron = (RAPTOR::g_assist_pos > 0.0f) ? RAPTOR::g_assist_pos : static_cast<float>(limit(RAPTOR::aileron_animation_command, -1, 1));
        const float target_right_aileron = (RAPTOR::g_assist_pos > 0.0f) ? RAPTOR::g_assist_pos : static_cast<float>(limit(-RAPTOR::aileron_animation_command, -1, 1));
        ...
        if (RAPTOR::flaps_pos > 0.0f) {
            ...
            const float flap_target_left = static_cast<float>(limit(-RAPTOR::aileron_animation_command, -1, 1));
            const float flap_target_right = static_cast<float>(limit(RAPTOR::aileron_animation_command, -1, 1));
